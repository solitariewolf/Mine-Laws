...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['id'])) {
    ...
    $stmt = $conn->prepare("SELECT Texto_Original, Novo_Texto FROM votacoes_leis WHERE ID = ?");
    ...
    // Verifique se a lei já existe na tabela "leis"
    $stmt = $conn->prepare("SELECT ID FROM leis WHERE ID = ?");
    $stmt->bind_param("i", $lei_id);
    $stmt->execute();
    $result = $stmt->get_result();
    ...
    if ($result->num_rows > 0) {
        ...
        $stmt = $conn->prepare("UPDATE leis SET Texto = ? WHERE ID = ?");
        $stmt->bind_param("si", $novo_texto, $lei_id);
    } else {
        // A lei não existe, insira uma nova linha
        $stmt = $conn->prepare("INSERT INTO leis (ID, Texto) VALUES (?, ?)");
        $stmt->bind_param("is", $lei_id, $texto_original);
    }

    if ($stmt->execute()) {
        // Atualize a lei para ser promulgada
        $stmt = $conn->prepare("UPDATE votacoes_leis SET Promulgado = 'sim' WHERE ID = ?");
        $stmt->bind_param("i", $lei_id);
        if ($stmt->execute()) {
