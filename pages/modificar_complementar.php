...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['lei_existente']) && isset($_POST['nova_lei'])) {
    ...
    $stmt = $conn->prepare("SELECT Texto FROM complementar WHERE ID = ?");
    ...
    $stmt = $conn->prepare("SELECT ID FROM votacoes_leis_complementares WHERE ID = ?");
    $stmt->bind_param("i", $lei_id);
    $stmt->execute();
    $result = $stmt->get_result();
    ...
    if ($result->num_rows > 0) {
        ...
        $stmt = $conn->prepare("UPDATE votacoes_leis_complementares SET Texto_Original = ?, Novo_Texto = ?, Votos_Positivos = 0, Votos_Negativos = 0, Total_Votos = 0, Arquivado = 'não', Promulgado = 'não' WHERE ID = ?");
        $stmt->bind_param("ssi", $texto_original, $novo_texto, $lei_id);
    } else {
        // A lei não existe, insira uma nova lei
        $stmt = $conn->prepare("INSERT INTO votacoes_leis_complementares (ID, Texto_Original, Novo_Texto, Votos_Positivos, Votos_Negativos, Total_Votos, Arquivado, Promulgado) VALUES (?, ?, ?, 0, 0, 0, 'não', 'não')");
